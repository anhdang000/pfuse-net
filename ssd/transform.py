"""
Modified from work of @author: Viet Nguyen <nhviet1009@gmail.com> to add local pattern image
by TuyenNQ - s1262008@u-aizu.ac.jp
"""
import random
...
class SSDCropping(object):
    ...
    def __call__(self, img, lp_img, img_size, bboxes, labels):
        ...
        while True:
            ...
            if mode is None:
                return img, lp_img, img_size, bboxes, labels
            ...
            for _ in range(1):
                ...
                lp_img = lp_img.crop((left_idx, top_idx, right_idx, bottom_idx))
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, img, lp_img, bboxes):
        if random.random() < self.prob:
            bboxes[:, 0], bboxes[:, 2] = 1.0 - bboxes[:, 2], 1.0 - bboxes[:, 0]
            return img.transpose(Image.FLIP_LEFT_RIGHT),lp_img.transpose(Image.FLIP_LEFT_RIGHT) , bboxes
        return img, lp_img,bboxes
...
class SSDTransformer(object):
    def __init__(self, dboxes, size=(300, 300), val=False):
        ...
        # Argumentation for Local Pattern
        self.lp_img_trans = transforms.Compose([
            transforms.Resize(self.size),
            transforms.ToTensor()
        ])
